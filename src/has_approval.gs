...
function ADD_HAS_SUBMISSION_TO_HAS(submissionDetailsArray, isLegacySubmission, isEventSubmission=false, isIncogSubmission=false) {
  ...
  if (submissionScore >= MINIMUM_SCORE_FOR_HIGHEST_ARRAS_SCORES) {
    ...
    else if (isIncogSubmission) {
      newHASArray.push(IAS_INDICATION_CHARACTER);
    }
...
function ADD_SCORE_TO_IAS(submissionDetailsArray, editedCell, isOnlyIasSubmission=true) {

  // destructure array into individual variables
  const [submissionScore, submissionPlayerName, submissionProofLink, submissionTank, submissionGamemode, submissionSpecialSubmission] = submissionDetailsArray;
  
  // add to HAS the normal way, but 2nd argument tells the ADD_TO_HAS function that its NOT a legacy one
  //    and 3rd argument says that it IS an incog arras scores submission
  // and to append an extra character in last column that tells spreadsheet to sort the score into the Legacy Sheet
  // if this function returns true, that means that everything went well and score was added
  if (ADD_HAS_SUBMISSION_TO_HAS(submissionDetailsArray, false, false, true) && isOnlyIasSubmission) {
    Browser.msgBox("INCOG HAS SUBMISSION APPROVED", `${PRINT_SUBMISSION_DETAILS(submissionDetailsArray)} has been added to Incog Highest Arras Scores`, Browser.Buttons.OK);
    editedCell.setValue(APPROVED_STATUS_CHARACTER);
    editedCell.setNote("Approved For IAS Only");
  }
  // else means that the function returned false, and that the score was not high enough for Incog HAS
  else if (isOnlyIasSubmission) {
    Browser.msgBox("INCOG HAS SUBMISSION REJECTED", `${PRINT_SUBMISSION_DETAILS(submissionDetailsArray)} is lower than the minimum score needed for Incog Arras Scores, currently ${FORMAT_SCORE(MINIMUM_SCORE_FOR_INCOG_ARRAS_SCORES)}`, Browser.Buttons.OK);
    editedCell.setValue(REJECTED_STATUS_CHARACTER);
  }
}
